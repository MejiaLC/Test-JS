...
const getHour = () => {
  ...
  let hour = (document.querySelector(".hour").textContent = fileZeros(
    fecha.getHours()
  ));
  let min = (document.querySelector(".min").textContent = fileZeros(
    fecha.getMinutes()
  ));
  let seconds = (document.querySelector(".sec").textContent = fileZeros(
    fecha.getSeconds()
  ));

  if (hour == 00 && min == 00 && seconds == 00) {
    alert("Ya es media noche");
  }
};
